return function(G)
	-- TODO: lua 5.0 does not support meta index chain ... if we dont change the real _G we need to create a new one and copy everything into it
	local ok, package = pcall(require,"package")
	--local ok, string = pcall(require,"string")
	if string.len(G._VERSION)~=7 and string.sub(G._VERSION,1,7)=="Lua 5.0" then
		G._LONGVERSION=G._VERSION
		G._VERSION="Lua 5.0"
	end
